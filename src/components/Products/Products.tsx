    ...
    <div className={styles.products}>
      {data?.map((item) => (
        <div key={item.id} className={styles.products__block}>
          <img src={item.image} alt={item.title} className={styles.products__image} />
          <p className={styles.products__article}>Артикул: {item.id}</p>
          <h3 className={styles.products__title} >{item.title}</h3>
          <h4 className={styles.products__price}>{item.price.toLocaleString("RU-ru")} ₽</h4>
          <div className={styles['products__block-bottom']}>
            <div className={styles.products__left}>
              <button className={styles['products__add-to-cart']}>
                <FiShoppingCart className={styles['products__add-to-cart-icon']} />
                <p>Купить</p>
              </button>
            </div>
            <div className={styles.products__rigth}>
              <button className={cn(
                styles['products__favorites-button'],
              ...
              >
               ...
               {
                favorites.data.some(favoritesItem => favoritesItem.id === item.id ) ? (
                  <MdFavorite style={{color: "red"}} className={styles['products__favorites-icon']} />
                ) : (
<MdFavoriteBorder className={styles['products__favorites-icon']} />
                )
               }                     
